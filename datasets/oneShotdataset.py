...
#NlabelTovector = torch.load('/home/zitian/data/miniImagenet/Nlabel2vec.t7')
np.random.seed(2191)  # for reproducibility

pathminiImageNet = '/home/zitian/data/miniImagenet/'
...
class miniImagenetOneshotDataset(data.Dataset):
    def __init__(self, dataroot = '/home/zitian/data/miniImagenet', type = 'train',ways=5,shots=1,test_num=1,epoch=100):
        ...
        if type == 'train' or type == 'val':
            self.data = dict(loadSplit(splitFile = os.path.join(dataroot,'train' + '.csv')).items() + loadSplit(splitFile = os.path.join(dataroot,'val' + '.csv')).items())
        else:
    ...
    def __getitem__(self, index):
        '''
        if self.type == 'val':
            c = self.valbelong[index]
            File = self.valFiles[index]
        else:
            c = self.belong[index]
...
#for j in range(5):
'''
for i,(supportInputs,supportLabels,testInputs,testLabels) in tqdm(enumerate(dataTrain)):
    haha = 1
    #print(i,supportInputs.size(),supportLabels.size(),testInputs.size(),testLabels.size())
    #print(testLabels)
    #if i==0:
    #    print(supportInputs[0,0:3])
